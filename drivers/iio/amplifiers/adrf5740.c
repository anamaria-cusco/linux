...
static struct adrf5740_chip_info adrf5740_chip_info = {
		...
		.gain_min = 0,
		.gain_max = 22,
		.default_gain = 15, /* set default attenuation 22dB*/
};
			     ...
			     long mask)
{
	...
	case IIO_CHAN_INFO_HARDWAREGAIN:
		db_gain = st->raw_gain << 1;
		if (st->raw_gain & BIT(3))
			db_gain -= 8;
			      ...
			      long mask)
{
	...
	switch (mask) {
	case IIO_CHAN_INFO_HARDWAREGAIN:
		if (abs(db_gain) & BIT(4))
			st->raw_gain = (abs(db_gain) + 8) >> 1;
		else
			st->raw_gain = abs(db_gain) >> 1;
...
static int adrf5740_probe(struct platform_device *pdev)
{
	...
	st->gpios = devm_gpiod_get_array(&pdev->dev, "ctrl", GPIOD_OUT_HIGH);
	if (IS_ERR(st->gpios))
