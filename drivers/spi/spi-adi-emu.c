...
/* CNVST get reset to 0 at the end of conversion*/
#define CNVST			BIT(0)
...
/* Bit 3,2,1 are used for test mode selection */
...
/* First register - write only, second - 8 bits writable..*/
const u8 wr_mask[9] = {
	0x0, 0xff, POWER_DOWN, CNVST, 0xf, 0xff, 0xf, 0xff, TEST_MODE(7)
};

/* 5 test modes, ADC Test Modes Register BIT(3) BIT(2) BIT (1) */
enum adi_emu_test_mode {
	TEST_OFF, /* normal operation*/
	TEST_MIDSCALE,
	TEST_POSITIVE,
	TEST_NEGATIVE,
	TEST_RAMP,
...
/* This is a sine wave 128 samples*/
				...
				struct spi_transfer *transfer)
{
	...
	u16 ch1_data;
/*
* The master sends a command byte to the slave,
* indicating the register it wants to read.
* The slave responds with the data from that register.
* But here is emulated, not actually sent.
*	struct spi_transfer transfer = {
*		.tx_buf = &command_byte,
*		.len = 1,  // Only sending the command byte for read op
*	};
*/
